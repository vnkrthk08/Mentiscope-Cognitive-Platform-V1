...
function directNormalize(value: number, min: number, max: number) {
  if (value >= max) return 100;
  if (value <= min) return 0;
  return Math.round(((value - min) / (max - min)) * 100);
}

function calculateModuleScore(moduleEvents: any[]) {
...
function calcRecoveryResilience(events: any[]) {
  ...
  const avgRecoveryLatency = recoveryEvents.length > 0
    ? recoveryEvents.reduce((a: number, b: any) => a + b.recoveryLatency, 0) / recoveryEvents.length
  ...
  const totalRetries = retries.reduce((sum: number, e: any) => sum + e.retryCount, 0);
...
function calcStressTolerance(events: any[]) {
  const module2Events = events.filter(e => e.module === 2);

  const panicEvents = module2Events.filter(e => e.eventType === 'PANIC_CLICK');
  const totalPanicClicks = panicEvents.reduce((sum: number, e: any) => sum + (e.panicClicks || 1), 0);
  ...
  const avgStressRT = stressRT.length > 0
    ? stressRT.reduce((a: number, b: number) => a + b, 0) / stressRT.length
    : THRESHOLDS.maximumReactionTime;
...
function calcAdaptationPersistence(events: any[]) {
  ...
  const avgAdaptation = adaptationEvents.length > 0
    ? adaptationEvents.reduce((a: number, b: any) => a + b.adaptationRate, 0) / adaptationEvents.length
    : 0.5;
  ...
  const totalRetries = allRetries.reduce((sum: number, e: any) => sum + e.retryCount, 0);
...
function calcDecisionStability(events: any[]) {
  const stabilityEvents = events.filter(e => e.decisionStability != null);
  const avgStability = stabilityEvents.length > 0
    ? stabilityEvents.reduce((a: number, b: any) => a + b.decisionStability, 0) / stabilityEvents.length
    : 0.5;
  ...
  if (decisions.length >= 4) {
    ...
    errorEscalation = secondErrors - firstErrors;
  }
...
export function calculateScore(events: any[], sessionId: string, studentId: string) {
  if (events.length === 0) {
    return null;
  }
  ...
  const avgReactionTime = allReactionTimes.length > 0
    ? Math.round(allReactionTimes.reduce((a: number, b: number) => a + b, 0) / allReactionTimes.length)
    : 0;

  const totalPanicClicks = events
    .filter(e => e.eventType === 'PANIC_CLICK')
    .reduce((sum: number, e: any) => sum + (e.panicClicks || 1), 0);
  ...
  const avgRecoveryLatency = recoveryLatencies.length > 0
    ? Math.round(recoveryLatencies.reduce((a: number, b: number) => a + b, 0) / recoveryLatencies.length)
    : 0;
  ...
  const avgAdaptationRate = adaptationRates.length > 0
    ? Math.round((adaptationRates.reduce((a: number, b: number) => a + b, 0) / adaptationRates.length) * 100) / 100
    : 0;

  const totalRetries = events.reduce((sum: number, e: any) => sum + (e.retryCount || 0), 0);
  ...
  events.forEach(e => {
    if (e.eventType === 'SCENARIO_COMPLETED') {
       ...
       if (e.metadata && e.metadata.livesAtRisk) {
         livesLost += Math.floor(e.metadata.livesAtRisk * 0.5);
       }
    }
  });
