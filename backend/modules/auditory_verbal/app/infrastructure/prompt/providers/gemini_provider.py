...
class GeminiProvider(LLMProvider):
    """Google Gemini generation provider integration."""

    def __init__(self, api_key: str = "", default_model: str = "gemini-1.5-pro"):
        self.api_key = api_key or settings.GEMINI_API_KEY
        self.default_model = settings.GEMINI_MODEL or default_model
    ...
    async def generate(self, system_prompt: str = None, user_prompt: str = None, prompt_text: str = None, options: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        # Call Gemini via OpenAI compatibility endpoint
        headers = {
        ...
        if not choices:
            raise ValueError(f"Gemini returned empty choices: {raw_data}")
        ...
        # Return OpenAI format and APOS format
        return {
            "choices": choices,
            "usage": usage,
            "id": raw_data.get("id", f"gemini-req-{uuid.uuid4()}"),
            "model": model,
    ...
    async def health_check(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(f"https://generativelanguage.googleapis.com/v1beta/models?key={api_key}", timeout=5.0)
    ...
    def estimate_cost(self, model_name: str, input_tokens: int, output_tokens: int) -> float:
        # Gemini pricing: $0.00125 / 1K input, $0.00375 / 1K output
        input_cost = (input_tokens / 1000.0) * 0.00125
        output_cost = (output_tokens / 1000.0) * 0.00375
