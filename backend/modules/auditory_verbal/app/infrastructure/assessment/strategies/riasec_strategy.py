...
class RIASECStrategy(FrameworkScoringStrategy):
    """Calculates career interest profiles scoring from RIASEC construct evaluations."""

    def calculate(self, profiles: List[ConstructProfile], policy: ScoringPolicy) -> FrameworkResult:
        riasec_profiles = [p for p in profiles if p.framework.upper() == "RIASEC"]
        if not riasec_profiles:
            raise ValueError("No RIASEC profiles available for scoring.")
        ...
        avg_raw = raw_sum / len(riasec_profiles)
        avg_conf = conf_sum / len(riasec_profiles)
        norm_avg = normalizer.normalize(avg_conf)

        summary = f"Scored RIASEC framework across {len(riasec_profiles)} interest types."
    ...
    def validate(self, profiles: List[ConstructProfile], policy: ScoringPolicy) -> List[str]:
        ...
        for p in riasec_profiles:
            if p.confidence.confidence_score < 0.0 or p.confidence.confidence_score > 1.0:
                errors.append(f"RIASEC construct '{p.construct_name}' score must range between 0.0 and 1.0.")
        return errors
pre=1.0
