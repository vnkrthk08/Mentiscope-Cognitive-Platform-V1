...
class EmotionalRegulationStrategy(FrameworkScoringStrategy):
    """Calculates emotional & behavioral regulation scores from construct profile evaluations."""

    def calculate(self, profiles: List[ConstructProfile], policy: ScoringPolicy) -> FrameworkResult:
        reg_profiles = [p for p in profiles if p.framework.upper() == "EMOTIONAL_REGULATION"]
        if not reg_profiles:
            raise ValueError("No EMOTIONAL_REGULATION profiles available for scoring.")
        ...
        avg_raw = raw_sum / len(reg_profiles)
        avg_conf = conf_sum / len(reg_profiles)
        norm_avg = normalizer.normalize(avg_conf)

        summary = f"Scored EMOTIONAL_REGULATION framework across {len(reg_profiles)} indicators."

        return FrameworkResult(
            framework="EMOTIONAL_REGULATION",
            raw_score=avg_raw,
    ...
    def validate(self, profiles: List[ConstructProfile], policy: ScoringPolicy) -> List[str]:
        ...
        for p in reg_profiles:
            if p.confidence.confidence_score < 0.0 or p.confidence.confidence_score > 1.0:
                errors.append(f"EMOTIONAL_REGULATION construct '{p.construct_name}' score must range between 0.0 and 1.0.")
        return errors
pre=1.0
