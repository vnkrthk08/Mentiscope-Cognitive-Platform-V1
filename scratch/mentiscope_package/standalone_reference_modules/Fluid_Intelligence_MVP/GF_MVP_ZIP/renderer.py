...
class SVGRenderer:
    ...
    def render_symbol(self, symbol: Symbol, *, x: float = 0, y: float = 0) -> str:
        ...
        stroke_width = cfg.stroke_widths[symbol.border.value]
        ...
        if symbol.fill is FillStyle.HATCHED:
            hatch = (
                ...
                f'<rect width="7" height="7" fill="white"/><line x1="0" y1="0" x2="0" y2="7" stroke="{escape(symbol.color)}" stroke-width="3"/></pattern></defs>'
            )
        ...
        return (
            ...
            f'{primitive} fill="{fill}" stroke="{escape(cfg.stroke_color)}" stroke-width="{stroke_width}" opacity="{symbol.opacity:.2f}"{dash}/></g></g>'
        )
