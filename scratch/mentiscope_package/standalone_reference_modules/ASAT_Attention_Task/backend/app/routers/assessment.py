"""
ASAT – MentiScope Standard Assessment Endpoints
...
from app.schemas import Session as SessionModel, Score, Event, Student
...
router = APIRouter(prefix="/api", tags=["MentiScope Assessment"])
...
async def submit_answer(
    ...
    # Log the event — same fields as existing event logging
    event = Event(
        student_id=student.student_id,
...
async def finish_assessment(
    ...
    # Save scores — same upsert logic as sessions.py
    if payload.scores:
