...
class CsrAssessmentService:
    ...
    def answer(self, session_id: str, question_id: str, answer: str, duration_ms: int) -> dict:
        ...
        if position >= engine.total_trials():
            return {"status": "success", "isCorrect": correct, "feedback": self._feedback(task, correct, item), "nextQuestion": None}

        next_question = self._load_trial(session_id, position)
        return {"status": "success", "isCorrect": correct, "feedback": self._feedback(task, correct, item), "nextQuestion": next_question}
    ...
    @staticmethod
    def _feedback(task: str, correct: bool, item: dict = None) -> str:
        if correct:
            return {
        ...
        if item is not None and task in ["auditory", "distractor", "sequential"]:
            if task == "sequential":
                expected = ",".join(engine.expected_sequence_order(item))
                distractors = engine._sequential_distractor_orders(item)
                options = [expected] + distractors
                seed = sum(ord(c) for c in item["id"])
                rotated = options[seed % len(options):] + options[:seed % len(options)]
                correct_ans = expected
                options_list = rotated
            else:
                correct_ans = item["answer"]
                options_list = item.get("options", [])
            
            if correct_ans in options_list:
                idx = options_list.index(correct_ans)
                letter = chr(65 + idx)
                return f"Incorrect. The correct option is {letter}"
