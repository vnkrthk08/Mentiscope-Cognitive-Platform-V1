"""
ASAT â€“ SQLAlchemy ORM Table Models
...
PostgreSQL differences from original MySQL:
  - AUTO_INCREMENT â†’ identity columns (autoincrement=True)
  - JSON â†’ JSONB (better indexing in PostgreSQL)
  - BOOLEAN is native (no TINYINT workaround)
  - TIMESTAMP DEFAULT CURRENT_TIMESTAMP â†’ server_default=func.now()
...
from sqlalchemy import (
    ...
    ForeignKey, func, DateTime, JSON
)
from sqlalchemy.orm import relationship

from modules.gsm.database import Base


class Faculty(Base):
    """
    Faculty authentication table.
    Translated from: db.js â†’ CREATE TABLE IF NOT EXISTS faculty
    """
...
class Student(Base):
    ...
    Translated from: db.js â†’ CREATE TABLE IF NOT EXISTS students
    """
...
class Session(Base):
    ...
    Translated from: db.js â†’ CREATE TABLE IF NOT EXISTS sessions
    """
...
class Event(Base):
    ...
    Translated from: db.js â†’ CREATE TABLE IF NOT EXISTS events
...
class Score(Base):
    ...
    Translated from: db.js â†’ CREATE TABLE IF NOT EXISTS scores
    """
    ...
    module_results = Column(JSON, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class Report(Base):
    ...
    Translated from: db.js â†’ CREATE TABLE IF NOT EXISTS reports
    """
