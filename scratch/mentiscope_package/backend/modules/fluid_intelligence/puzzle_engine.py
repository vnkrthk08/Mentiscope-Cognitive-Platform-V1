...
class PuzzleGenerator:
    ...
    def generate(self, level: DifficultyLevel) -> Puzzle:
        ...
        for idx, opt in enumerate(options):
            opt_id = f"opt_{idx}"
            if opt is correct or _fingerprint(opt.card) == _fingerprint(correct_card):
                correct_option_id = opt_id
            stable_options.append(AnswerOption(card=opt.card, misconception=opt.misconception, option_id=opt_id))
        question = Question(question_input, tuple(stable_options), correct_option_id or "opt_0")
        return Puzzle(
