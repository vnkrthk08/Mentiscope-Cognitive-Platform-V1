...
import { Eye, RotateCw, AlertTriangle } from "lucide-react";
...
class GVErrorBoundary extends (Component as any) {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center p-6 text-center rounded-2xl border border-amber-800/40 bg-amber-950/20 text-amber-300">
          <AlertTriangle className="h-6 w-6 text-amber-400 mb-2" />
          <p className="text-xs font-bold">{this.props.fallbackText || "Visual diagram preview ready."}</p>
        </div>
      );
    }
...
function ShapeGraphic({ data, label }: { data?: Record<string, unknown>; label?: string }) {
  const safeData = data || {};
  const shape = ((safeData.shape || safeData) as { cells?: number[][]; color?: string }) || {};
  const cells = Array.isArray(shape.cells) ? shape.cells : [];
  const rotation = Number(safeData.rotation || 0);
  const mirror = Boolean(safeData.mirror);
  const size = 26;

  const xCoords = cells.map((cell) => (Array.isArray(cell) ? Number(cell[0] || 0) : 0));
  const yCoords = cells.map((cell) => (Array.isArray(cell) ? Number(cell[1] || 0) : 0));
  const maxX = Math.max(1, ...xCoords) + 1;
  const maxY = Math.max(1, ...yCoords) + 1;
  const viewBoxSize = Math.max(maxX, maxY) * size + 12;

  return (
    <div className="flex flex-col items-center gap-2">
      <svg viewBox={`0 0 ${viewBoxSize} ${viewBoxSize}`} className="h-28 w-28" role="img" aria-label={label || "Geometric figure"}>
        <g transform={`translate(${viewBoxSize / 2} ${viewBoxSize / 2}) rotate(${rotation}) scale(${mirror ? -1 : 1} 1) translate(${-(maxX * size) / 2} ${-(maxY * size) / 2})`}>
          {cells.map((cell, index) => {
            if (!Array.isArray(cell)) return null;
            const cx = Number(cell[0] || 0);
            const cy = Number(cell[1] || 0);
            return (
              <rect
                key={`${cx}-${cy}-${index}`}
                x={cx * size + 1}
...
function HoleGrid({ holes, label }: { holes?: number[][]; label: string }) {
  const validHoles = Array.isArray(holes) ? holes : [];
  return (
    <div className="grid h-28 w-28 grid-cols-4 overflow-hidden rounded-lg border-2 border-slate-300 bg-white" role="img" aria-label={label}>
      {Array.from({ length: 16 }).map((_, index) => {
        ...
        const col = index % 4;
        const punched = validHoles.some((pt) => Array.isArray(pt) && pt[0] === col && pt[1] === row);
        return (
  ...
  return (
    <svg viewBox="0 0 100 100" className="h-28 w-full max-w-40" role="img" aria-label={label}>
      {validSegments.map((segment, index) => {
        if (!Array.isArray(segment) || !Array.isArray(segment[0]) || !Array.isArray(segment[1])) return null;
        return (
          <line
            key={index}
            x1={segment[0][0]}
...
function MapGrid({ map, label }: { map?: string[][]; label: string }) {
  const validMap = Array.isArray(map) && map.length > 0 ? map : [["grass"]];
  const columns = validMap[0]?.length || 1;
  return (
    <div
      className="grid aspect-square w-full max-w-md overflow-hidden rounded-2xl border-4 border-white shadow-lg ring-1 ring-slate-200"
      style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}
      role="img"
      aria-label={label}
    >
      {validMap.flatMap((row, rowIndex) =>
...
function rotateMatrix<T>(matrix: T[][], degrees: number): T[][] {
  ...
  for (let turn = 0; turn < turns; turn += 1) {
    if (output.length === 0 || !Array.isArray(output[0])) break;
...
function PiecePreview({ piece, rotation }: { piece?: Record<string, unknown>; rotation: number }) {
  if (!piece) return <div className="h-16 w-16 bg-slate-800 rounded-lg" />;
...
function SingleChoiceRenderer({ item, disabled, onChange, onBehavior }: RendererProps) {
  ...
  return (
    <GVErrorBoundary fallbackText="Visual stimulus question ready.">
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2 text-xs font-semibold text-teal-800">
                {((stimulus.folds || []) as Array<Record<string, unknown>>).map((fold, index) => (
                  <span key={index} className="rounded-full border border-teal-200 bg-white px-3 py-1">
                    Fold {index + 1}: {String(fold?.axis || "")} → {String(fold?.direction || "")}
                  </span>
                ))}
              </div>
            </div>
          )}
          {item.subtest_id === "hidden_figures" && (
            <SegmentGraphic segments={(stimulus.target_segments || []) as Segment[]} label="Target figure to locate" />
          )}
        </div>
            ...
            {options.map((option, index) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-40 rounded-2xl border-2 p-4 transition focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-300 cursor-pointer ${
                    active ? "border-teal-600 bg-teal-50 shadow-md ring-2 ring-teal-500/20 scale-[1.02]" : "border-slate-200 bg-white hover:border-teal-300 hover:shadow-sm"
                  }`}
                >
                  <span className="mb-2 block text-left text-xs font-bold uppercase tracking-wider text-slate-500">
                    Option {index + 1}
                  </span>
                  {item.subtest_id === "mental_rotation" && (
                    <ShapeGraphic data={payload} label={`Option ${index + 1} geometric figure`} />
                  )}
                  {item.subtest_id === "paper_folding" && (
                    <div className="flex justify-center">
                      <HoleGrid holes={(payload.holes || []) as number[][]} label={`Option ${index + 1} hole pattern`} />
                    </div>
                  )}
                  {item.subtest_id === "hidden_figures" && (
                    <SegmentGraphic segments={(payload.segments || []) as Segment[]} label={`Option ${index + 1} complex figure`} />
                  )}
                </button>
              );
            })}
...
function MapPlacementRenderer({ item, disabled, onChange, onBehavior }: RendererProps) {
  ...
  useEffect(() => {
    const initial = Object.fromEntries(
      (pieces || []).map((piece) => [String(piece?.piece_id || ""), Number(piece?.initial_rotation || 0)])
    );
  ...
  const publish = (nextPlacements: Record<number, Placement>, nextRotations = rotations) => {
    ...
    const responsePlacements = Object.fromEntries(
      Object.values(nextPlacements).map((placement) => [
        placement.pieceId,
        { slot_index: placement.slotIndex, rotation: nextRotations[placement.pieceId] || 0 },
      ])
    );
    onChange({
      response: complete ? { placements: responsePlacements } : { placements: responsePlacements },
  ...
  if (studyLeft > 0) {
    return (
      <div className="flex flex-col items-center gap-6 rounded-3xl border border-teal-100 bg-teal-50/40 p-6 sm:p-8" aria-live="polite">
        <div className="flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-bold text-teal-700 shadow-sm">
          <Eye className="h-4 w-4" /> Study the complete map: {studyLeft}s
        </div>
        <MapGrid map={map} label="Complete map to remember" />
        <p className="max-w-xl text-center text-sm text-slate-600">
          Notice roads, water, landmarks, and the position of each region. The reference will hide before placement begins.
        </p>
      </div>
    );
  }
  ...
  return (
    <GVErrorBoundary fallbackText="Map reconstruction board ready.">
            ...
            {Array.from({ length: cols * rows }).map((_, slotIndex) => {
              ...
              return (
                <button
                  ...
                  className={`aspect-square min-h-24 rounded-2xl border-2 border-dashed p-2 transition focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-300 cursor-pointer ${
                    placement ? "border-teal-500 bg-white shadow-sm" : selectedPieceId ? "border-teal-300 bg-teal-50 hover:border-teal-500" : "border-slate-300 bg-slate-50"
                  }`}
                >
                  {placement ? (
                    <PiecePreview piece={pieceById[placement.pieceId]} rotation={rotations[placement.pieceId] || 0} />
                  ) : (
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">
            {pieces.map((piece, index) => {
              const pieceId = String(piece?.piece_id || index);
              const active = selectedPieceId === pieceId;
              const placed = placedPieceIds.has(pieceId);
              return (
                <div key={pieceId} className={`rounded-2xl border p-3 ${active ? "border-teal-500 bg-teal-50 shadow-md ring-2 ring-teal-500/20" : "border-slate-200 bg-white"}`}>
                  <button
                    type="button"
                    disabled={disabled}
                    onClick={() => selectPiece(pieceId)}
                    aria-pressed={active}
                    className="w-full rounded-xl focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-300 cursor-pointer"
                  >
                    <span className="mb-2 block text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">
                      Piece {index + 1}{placed ? " · placed" : ""}
                    </span>
                  ...
                  <button
                    type="button"
                    disabled={disabled}
                    onClick={() => rotatePiece(pieceId)}
                    className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-300 cursor-pointer"
                    aria-label={`Rotate piece ${index + 1} clockwise`}
                  >
          ...
          <p className="mt-4 text-xs leading-relaxed text-slate-500">
            Accessible placement: select a piece, rotate it using the button, then choose a numbered slot. Selecting a placed piece lets you move it.
          </p>
        </div>
      </div>
    </GVErrorBoundary>
