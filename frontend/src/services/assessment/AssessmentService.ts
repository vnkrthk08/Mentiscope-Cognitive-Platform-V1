...
export class AssessmentService {
  ...
  static async syncExternalModule(sessionId: string, studentId: string): Promise<AssessmentSession | null> {
    try {
      const res = await fetch(`/api/sessions/sync-external?session_id=${encodeURIComponent(sessionId)}&student_id=${encodeURIComponent(studentId)}`);
      if (res.ok) {
      ...
      console.warn("External sync request failed:", e);
    }
    return null;
  }
