...
export default function StudentDashboard({ user, onNavigate, onStartAssessment }: StudentDashboardProps) {
  ...
  // Automated Score Sync Notification State
  ...
  React.useEffect(() => {
    ...
    if (activeSess?.sessionId) {
      fetch(`/api/sessions/sync-external?session_id=${encodeURIComponent(activeSess.sessionId)}&student_id=${encodeURIComponent(user.id)}`)
        .then(res => res.ok ? res.json() : null)
        .then(data => {
    ...
    if (modParam && scoreParam) {
      ...
      if (!isNaN(num)) {
        ...
        if (updated) {
          setSession(updated);
          setHistorySessions((prev) => [updated, ...prev.filter((s) => s.sessionId !== updated.sessionId)]);
          setScoreSuccessMsg(`Module ${modParam} score (${num}%) synchronized successfully.`);
        ...
        if (targetId && targetScore !== undefined) {
          ...
          if (!isNaN(num)) {
            ...
            if (updated) {
              setSession(updated);
              setHistorySessions((prev) => [updated, ...prev.filter((s) => s.sessionId !== updated.sessionId)]);
              setScoreSuccessMsg(`Module ${targetId} score (${num}%) synchronized via real-time channel.`);
              setTimeout(() => setScoreSuccessMsg(null), 3000);
            }
          }
        }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.status === "synced" && data.session) {
          ...
          setSession(data.session);
          setHistorySessions((prev) => [data.session, ...prev.filter((s) => s.sessionId !== data.session.sessionId)]);
          setScoreSuccessMsg(`Successfully synchronized ${data.moduleId || "Module 10"} score (${data.score}%) from external assessment.`);
          setTimeout(() => setScoreSuccessMsg(null), 3000);
          ...
          if (updated) {
            setSession(updated);
            setHistorySessions((prev) => [updated, ...prev.filter((s) => s.sessionId !== updated.sessionId)]);
            setScoreSuccessMsg(`Successfully synchronized Module 10 score (${data.score}%) from external assessment.`);
            setTimeout(() => setScoreSuccessMsg(null), 3000);
          }
        } else {
          setScoreSuccessMsg(data.message || "All external modules are up-to-date.");
          setTimeout(() => setScoreSuccessMsg(null), 3000);
        }
      }
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8 transition-colors duration-300">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            <button
              ...
              className="flex items-center justify-center gap-1.5 rounded-xl border border-indigo-200 dark:border-indigo-800/80 bg-indigo-50/70 dark:bg-indigo-950/40 px-4 py-3 text-sm font-semibold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition-all active:scale-[0.98] cursor-pointer shadow-xs disabled:opacity-50"
            >
              <RefreshCw className={`h-4.5 w-4.5 text-indigo-600 dark:text-indigo-400 ${syncingExternal ? "animate-spin" : ""}`} />
              <span>{syncingExternal ? "Syncing..." : "Sync External Modules"}</span>
            </button>
      ...
      {scoreSuccessMsg && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 rounded-2xl bg-slate-900 border border-emerald-500/50 p-4 text-xs font-semibold text-emerald-300 shadow-2xl shadow-emerald-500/10">
