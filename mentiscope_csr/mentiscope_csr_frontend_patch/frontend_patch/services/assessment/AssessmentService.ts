...
import { GSMService } from "../modules/gsm";
...
export class AssessmentService {
  ...
  static async startModule(moduleId: string, sessionId: string, studentId?: string): Promise<{ status: string; totalQuestions: number; question?: Question }> {
    ...
    if (moduleId === "gsm") return GSMService.start(sessionId, studentId);
  ...
  static async submitAnswer(
    ...
    if (moduleId === "gsm") return GSMService.answer(sessionId, payload);
  ...
  static async finishModule(
    ...
    if (moduleId === "gsm") return GSMService.finish(sessionId, answers);
  ...
  static async getModuleResult(moduleId: string, sessionId: string): Promise<{ moduleId: string; score: number }> {
    ...
    if (moduleId === "gsm") return GSMService.getResult(sessionId);
